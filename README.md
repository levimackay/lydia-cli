...
- **A real coding agent.** Reads files, searches your codebase (literal or
  ...
  shell commands, drives git — via Ollama's native tool calling.
...
deterministically first, then only uses the model to synthesize — it
...
email/Canvas/calendar/weather/stocks/news and open apps — never edits
...
next, and the reasoning behind past calls — check it before starting
