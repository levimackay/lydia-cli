...
`lydia-cli` is live on PyPI: https://pypi.org/project/lydia-cli/, and `pip
...
(OIDC) mechanism instead of a stored API token, so there is no secret to
leak, rotate, or accidentally commit. This is configured on PyPI's side
...
can never be re-uploaded, even after deleting it. If a release turns out
broken, ship a new version number, don't try to fix v0.1.0 in place.
